...
class MainMenuState extends MusicBeatState
{
	...
	{
		return Constants.VERSIONS.psych;
	}
	...
	override function create()
	{
		...
		var versionText:FlxText = new FlxText(12, FlxG.height - 4, 0, 12);
		versionText.scrollFactor.set();
		versionText.setFormat(Paths.font("vcr.ttf"), 16, FlxColor.WHITE, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		add(versionText);

		for (version in Reflect.fields(Constants.VERSIONS))
		{
			switch (version)
			{
				case 'funkin':
					versionText.text += "\nFunkin v" + Constants.VERSIONS.funkin;
				case 'psych':
					versionText.text += "\nPsych Engine v" + Constants.VERSIONS.psych;
				case 'neral':
					versionText.text += "\nNeral " + Constants.VERSIONS.neral;

				default:
					versionText.text += "\n" + version + " v" + Reflect.field(Constants.VERSIONS, version);
			}
